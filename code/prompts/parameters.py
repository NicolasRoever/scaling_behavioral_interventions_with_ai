...
INTERVIEW_PARAMETERS = {
    ...
    "T3_DIRECT_PERSUASION": {
    # META DATA (OPTIONAL):
    "_name": "T3_DIRECT_PERSUASION",
    "_description": "Motivational interview.",
    # OPTIONAL FEATURES:
    "moderate_answers": False,
    "moderate_questions": False,
    ...
    # INTERVIEW STRUCTURE:
    "first_question": FIRST_QUESTION,
    "first_ai_question_name": "persuasion_negatives_only",
    "global_mi_system_prompt": GLOBAL_PERSUASION_SYSTEM_PROMPT,
    "interview_plan": [
        {
            "question_name": "persuasion_negatives_only",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly acknowledge what the Interviewee just said about their social media use, including any apps or platforms they mentioned. "
                "Say that some people find that using social media more than they intend often comes with downsides, and ask whether this applies to them in any way. "
                "Do not provide examples yourself; let the Interviewee describe any downsides they notice. "
                "Do NOT offer solutions yet; your goal in this turn is only to gather examples of problems.\n"
                "RESPONSE CONTRACT: End with ONE open question.\n"
            ),
            "next_question": "persuasion_negatives_advice"
        },

        {
            "question_name": "persuasion_negatives_advice",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly restate, in your own words, one or two of the problems they mentioned, and gently acknowledge that these kinds of downsides are common among people who use social media a lot. "
                "Ask in a non-judgmental way whether they have ever tried anything to manage or limit their social media use before. "
                "Do not give examples at this point; let the Interviewee describe anything they have tried, if anything.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_positives_ask"
        },

        {
            "question_name": "persuasion_positives_ask",
            "system": (
                "Compose the next assistant message.\n"
                "Transition by saying that people often stick with high social media use because they feel they gain something from it. "
                "Ask what they see as the main benefits they get from using social media the way they do now "
                "(do not provide any examples.)\n"
                "RESPONSE CONTRACT: End with ONE open question.\n"
            ),
            "next_question": "persuasion_positives_counter"
        },

        {
            "question_name": "persuasion_positives_counter",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly acknowledge one or two of the benefits they mentioned, showing you understand why those matter to them. "
                "Then, for each benefit, ask if they see some potential flipsides. You can gently reframe here; for example, if they say social media keeps them connected with friends, "
                "you might ask whether there is also a flipside where it makes it harder to be fully present with people who matter most offline. "
                "Keep the tone firm but not hostile. "
                "Then ask how, taking both the upsides and these downsides into account, they now see the overall value of using social media as much as they currently do.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_evidence_harms"
        },

        {
            "question_name": "persuasion_evidence_harms",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly connect to what the Interviewee has already said about problems with social media in the 'Interview History' given above. "
                "Then add a short research line in simple, everyday language, consistent with the GLOBAL_PERSUASION_SYSTEM_PROMPT, such as: "
                "'In big studies where some people turned off or sharply limited social media for a few weeks, those who cut back tended to feel a bit happier, less lonely, and more time to connect with other people offline"
                "and many of them also ended up using social media less even after the experiment ended.' "
                "Use cautious language like 'tends to' or 'for many people' and do NOT mention technical research terms. "
                "Then ask whether this kind of evidence resonates with them or whether they find it surprising.\n"
                "RESPONSE CONTRACT: End with ONE open question.\n"
            ),
            "next_question": "persuasion_goals_dissonance"
        },

        {
            "question_name": "persuasion_goals_dissonance",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly mention the key potential negative effects you have discussed so far (for example distraction, sleep, or mood). "
                "Then ask how well their current social media habits fit with the kind of life they want, "
                "and in what ways their current habits do NOT fit with that life.\n"
                "RESPONSE CONTRACT: End with ONE open question.\n"
            ),
            "next_question": "persuasion_importance_scale"
        },

        {
            "question_name": "persuasion_importance_scale",
            "system": (
                "Compose the next assistant message.\n"
                "State that once people look at the downsides and the research, many conclude that cutting back is actually quite important. "
                ...
                "how important is it for you to reduce your social media time?'\n"
                "RESPONSE CONTRACT: End with exactly that question.\n"
            ),
            "next_question": "persuasion_importance_why_not_lower"
        },

        {
            "question_name": "persuasion_importance_why_not_lower",
            "system": (
                "Compose the next assistant message.\n"
                "If the last user message includes a number from 0 to 10 greater than 0, ask exactly: "
                "'Why is it a <that number> and not something lower?' Encourage them to explain why reducing their use matters at least to that degree. "
                "If the number is 0, briefly acknowledge it and ask what would have to happen in their life for that number to move above 0 in the future.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_confidence_scale"
        },

        {
            "question_name": "persuasion_confidence_scale",
            "system": (
                "Compose the next assistant message.\n"
                "Tell them that many people underestimate their ability to change habits until they actually try. "
                "Then ask the exact scaling question:\n"
                "'On a scale from 0 to 10, where 0 means \"not at all confident\" and 10 means \"extremely confident\", "
                "how confident are you that you could reduce your social media use if you really decided to?'\n"
                "RESPONSE CONTRACT: End with exactly that question.\n"
            ),
            "next_question": "persuasion_confidence_why_not_lower"
        },

        {
            "question_name": "persuasion_confidence_why_not_lower",
            "system": (
                "Compose the next assistant message.\n"
                "If the last user message includes a number from 0 to 10 greater than 0, ask exactly: "
                "'Why is it a <that number> and not something lower?' "
                "After they answer, explicitly point out that the reasons they gave show they already have some ability to change "
                "(for example discipline in other areas or past successes). "
                "If they answered 0, briefly acknowledge this and ask what small thing would need to be different for their confidence to rise even slightly above 0.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_why_no_change_yet"
        },

        {
            "question_name": "persuasion_why_no_change_yet",
            "system": (
                "Compose the next assistant message.\n"
                "Sum up briefly the reasons the Interviewee gave for change in the 'Interview History' above. "
                "Then ask directly: 'Given that you see benefits in cutting back and believe you could do it, what has stopped you from actually making a change so far?' "
                "After they answer, you may briefly frame these reasons as common and understandable, "
                "but also as obstacles that can often be managed with simple changes.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_concrete_plan"
        },

        {
            "question_name": "persuasion_concrete_plan",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly reflect the reasons they gave for not changing yet given in the 'Interview History', and acknowledge that it can be hard to change habits. "
                "Then propose one or two very concrete, simple plans that many people use successfully "
                "(for example, 'set a 30-minute daily limit for your main app' or 'no social media after 10 pm'), "
                "framing them as small experiments they could try for a week rather than rules they must follow. "
                "Make it clear that these are just options and that it's completely up to them what they choose. "
                "Then ask which of these they would be most willing to try over the next few days, "
                "or if they prefer a similar small step of their own.\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "persuasion_closing_takeaway"
        },

        {
            "question_name": "persuasion_closing_takeaway",
            "system": (
                "Compose the next assistant message.\n"
                "Briefly restate the main reasons they themselves gave for reducing social media use and the plan or idea they seemed most open to. "
                "Focus only on the arguments in favor of change, not on doubts. "
                "Then ask: 'What stands out most to you now about why reducing your social media use is a good idea for you?'\n"
                "RESPONSE CONTRACT: End with ONE clear question.\n"
            ),
            "next_question": "wrap_up"
        },

        {
            ...
            "system": (
                ...
                "Summarize the interview very briefly, focusing on: "
                "(1) the main ways their current social media use causes problems for them "
                "(for example distraction, sleep, mood, or time with people who matter), "
                "(2) the key benefits they expect from cutting back, and "
                "(3) the specific small step or experiment they said they would be willing to try. "
                "Clearly but respectfully reinforce that reducing social media is likely to help them move toward the kind of life they said they want "
                "(for example, more focus, better rest, more presence with family or friends). "
                "Explicitly affirm their autonomy with a short statement such as 'Of course, it's up to you what you decide to do with this.' "
                "Thank them for their time. Do NOT ask another question and do NOT introduce new advice.\n"
