...
responses and 2,290 matched time-use responses). The document
[docs/REPRODUCIBILITY_NOTES.md](../../docs/REPRODUCIBILITY_NOTES.md) records the input versions and validation.
...
corrected default scripts intact; see `../../docs/SUBMISSION_TABLE_AUDIT.md`.
...
The demand loader defaults to `df_merged_llm_category_expdemand_luna_v002.dta`.
`stats_expdemand.do` runs the shared subgroup SUR tests; optionally pass an
output CSV path to save their exact statistics. Public ID columns contain
anonymous release keys despite their legacy names.
