...
* NOW ONLY FOR THE VERIFIED (SCREENSHOT) SUBSAMPLE
foreach y in verified_prefered_time_w  {
    eststo: reg `y' i.T $controls_followup, r
...
esttab * , se b(3) nobase noomitted keep (*T) label starlevels(* 0.10 ** 0.05 *** 0.01) mtitles("\makecell{Ideal social\\ media time (min)}" "\makecell{Predicted social\\ media time (min)}" "\makecell{Actual social\\ media time (min)}" "\makecell{Verified social\\ media time (min)}") coeflabel(1.T "Change Talk (a)" 2.T "Decisional Balance (b)" 3.T "Direct Persuasion (c)") stats(N r2 controlmean controls p_amb_cha p_cha_per p_amb_per, fmt(%9.0fc %9.3f %9.3f %9.3f %9.3f %9.3f %9.3f) labels("Observations" "R\textsuperscript{2}" "Control group mean" "Controls" "p-value: a=b" "p-value: a=c" "p-value: b=c"))


esttab * using "${overleaf}/tables/tab_main_treatment_effects_sm_minutes.tex",  se b(3) nobase noomitted keep(*T) label starlevels(* 0.10 ** 0.05 *** 0.01) mtitles("\makecell{Ideal social\\ media time (min)}" "\makecell{Predicted social\\ media time (min)}" "\makecell{Actual social\\ media time (min)}" "\makecell{Verified social\\ media time (min)}") coeflabel(1.T "Change Talk (a)" 2.T "Decisional Balance (b)" 3.T "Direct Persuasion (c)") stats(N r2 controlmean controls p_amb_cha p_cha_per p_amb_per, fmt(%9.0fc %9.3f %9.3f %9.3f %9.3f %9.3f %9.3f) labels("Observations" "R\textsuperscript{2}" "Control group mean" "Controls" "p-value: a=b" "p-value: a=c" "p-value: b=c"))  booktabs fragment replace
