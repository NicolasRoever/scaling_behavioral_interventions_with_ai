...
## Experimenter demand and identifier checks

The Luna v002 demand input contains only an anonymous participant key and a
saved category code. Free-text study-purpose answers and the full private merge
are omitted. Reclassifying those answers requires the restricted survey text;
public reproduction uses the saved codes and makes no model calls.
